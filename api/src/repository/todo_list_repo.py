...
class TodoListMapper:
    ...
    def to_entity_with_todos(
        todo_list_model: TodoListModel, todo_models: list[TodoModel]
    ) -> TodoList:
        todo_list = TodoList(
            id=todo_list_model.id,
            title=todo_list_model.title,
        ...
        todo_list.set_todos(
            [TodoMapper.to_entity(todo_model) for todo_model in todo_models]
        )
        return todo_list
...
class TodoListRepo(BaseModel):
    ...
    async def fetch_list_by_todo_id(self, todo_id: str) -> TodoList | None:
        ...
        if todo_list_model:
            return TodoListMapper.to_entity_with_todos(
                todo_list_model[0], todo_list_model[0].todos
            )
        else:
            return None
    ...
    async def fetch_user_lists_with_todos(self, user: User) -> list[TodoList]:
        stmt = (
            select(TodoListModel)
            .where(TodoListModel.user_id == user.id)
            .options(selectinload(TodoListModel.todos))
        )
    ...
    async def update_list(self, todo_list_entity: TodoList):
        ...
        if (
            not (todo_list_entity.add_todos)
            and not (todo_list_entity.update_todos)
            and not (todo_list_entity.delete_todos)
        ):
            return
        ...
        if todo_list_entity.add_todos:
            ...
            for todo in todos:
                new_todo = TodoMapper.to_model(
                    todo_entity=todo, todo_list_model=todo_list_model
                )
