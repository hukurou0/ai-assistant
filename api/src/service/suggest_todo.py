...
class SuggestTodoService(BaseModel):
    ...
    def _find_free_times(self, events, min_duration: int = 15) -> list[FreeTimeVO]:
        ...
        for event in events:
            start_of_event = tz_tokyo.localize(
                ...
                if duration >= min_duration:
                    free_times.append(
                        FreeTimeVO(
                            ...
                            start=start_of_free_time,
                            end=start_of_event,
                        )
                    )
            end_of_event = tz_tokyo.localize(
                datetime.fromisoformat(
        ...
        if start_of_free_time < end:
            ...
            free_times.append(
                FreeTimeVO(duration=duration, start=start_of_free_time, end=end)
            )

        return free_times

    async def find_well_todos(self, user_id: str) -> list[SuggestTodoVO]:
            ...
            for i, todo in enumerate(todos):
                if todo.required_time:
                    if duration >= todo.required_time:
                        well_todo = SuggestTodoVO(free_time=free_time, todo=todo)
